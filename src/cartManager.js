...
export class CartManager{
    ...
    async addProductToCart(cid, pid) {
        ...
        if (typeof result == 'string') return `[404] Product whit ID=${pid} was not found`
        const cart = await this.getProductsFromCart(cid)
        if (typeof cart == 'string') return `[404] Cart whit ID=${cid} was not found`
        const productIndex = cart.products.findIndex(item => item.products === pid)
        if (productIndex > -1) {
